...
import { getGalleryMilestone, getImgByGalleryMilestone } from "../../api/apiMilestones";

export function ListenerGalleryMilestone({ id }) {
  ...
  useEffect(() => {
    async function loadGalleryAndImages() {
      try {
        const galleryData = await getGalleryMilestone(id);
        setGalleries(galleryData);

        const imagesData = await Promise.all(
          galleryData.map(async (gallery) => {
            const imagesByGallery = await getImgByGalleryMilestone(gallery.title_gallery);
    ...
    <div>
      {galleryImages.map((galleryData, galleryIndex) => (
        ...
        <div className="flex justify-center" key={galleryData.gallery.id}>
          ...
          <div className="flex  relative w-[700px]" >
            <div className="flex  w-full">
              {galleryData.images.map((img, imageIndex) => (
                ...
                >
                  <img className="w-[700px] h-[350px] sm:h-[500px] rounded-3xl" src={img.image_only} alt="" />
                </div>
              ))}
            </div>
            ...
            <div className="absolute bottom-4 md:w-[700px] flex md:justify-center sm:justify-center justify-center w-[450px]">
              <button className="p-1 rounded-full shadow bg-white/80 text-gray-800 hover:bg-white" 
              onClick={() => {navigate(`/Hitos/${id}/${galleryData.gallery.id}/${galleryData.gallery.title_gallery}`)}}>Galeria completa</button>
            </div>
          </div>
        </div>
        </div>
