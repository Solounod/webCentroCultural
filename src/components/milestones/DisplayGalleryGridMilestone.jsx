...
import { getImgByGalleryPagesMilestone } from "../../api/apiMilestones";

export function DisplayGalleryGridMilestone() {
        ...
        {images.map((image) => (
          <div key={image.id} className="">
            <div
              ...
              className={`${
                expandedImageId === image.id ? "h-[300px] md:w-[1000px] md:h-[600px] md:z-10 md:absolute  md:inset-6 md:hover:cursor-pointer" : "flex w-full sm:px-1 md:w-[300px] h-[300px] image-cover transform scale-100 md:hover:scale-110 hover:cursor-pointer "
              }   `}
            >
              <img
                className={`${
